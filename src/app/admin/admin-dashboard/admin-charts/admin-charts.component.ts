    ...
    barChartBrowser(): void {
      ...
      var mybarChart = new Chart(this.barCtx, {
        ...
        data: {
          datasets: [
            {
              backgroundColor: '#673ab7', // Set a single color for all bars
              borderColor: '#fff',
              borderWidth: 3,
              data: [
                { x: 'UI Itern', y: 30 },
      ...
      tooltip : {
        callbacks : {
          label : function (ctxLine:any) {
            let lab = ctxLine.dataset.label + ": " + ctxLine.dataset.data[ctxLine.dataIndex].labelText + " ~ " + ctxLine.dataset.data[ctxLine.dataIndex].y;
            return lab;
          },
